...
begin
    ...
    let g_E := g.map (algebra_map F E),
    let E' := polynomial.splitting_field g_E,
    let ι_FE := algebra_map F E,
    let ι_EE' := algebra_map E E',
    let ι_FE' := algebra_map F E',
    have comp_FEE' : ι_EE'∘ι_FE = ι_FE' := by ext; refl,
    have key : ∃ c : F, ∀ (α' : roots f E') (β' : roots g E'), ι_EE' β ≠ β' → ι_FE' c ≠ -(α' - (ι_EE' α))/(β' - (ι_EE' β)) := sorry,
    -- have key : ∃ c : F, ∀ α' : roots f E', ∀ β' : roots g E', ↑β' ≠ ι β → ι c ≠ -(α'-ι α)/(β'-ι β) := sorry,
    cases key with c hc,
    use c,
    let γ := α + c*β,
    let γ' := adjoin_simple.gen F γ,
    let ι_FFγ := algebra_map F F[γ],
    let ι_FγE := algebra_map F[γ] E,
    let g_Fγ := g.map ι_FFγ,
    let f_Fγ := f.map ι_FFγ,
    let f' : polynomial F[γ] := f_Fγ.comp(polynomial.C γ' - (polynomial.C (ι_FFγ c))*polynomial.X),
    let h := euclidean_domain.gcd f' g_Fγ,
    let h_E := h.map ι_FγE,
    have h_sep : h.separable :=
    begin
        have div := euclidean_domain.gcd_dvd_right f' g_Fγ,
        cases div with p mul,
        dsimp [← h] at mul,
        apply polynomial.separable.of_mul_left,
        rw ← mul,
    ...
    have h_map_separable : (h.map(algebra_map F[γ] E')).separable :=
    begin
        apply polynomial.separable.map,
        exact h_sep,
    end,
    sorry,
    -- have h_root : h_E.eval β = 0 :=
    -- begin
    --     dsimp [h_E],
    --     rw euclidean_domain.gcd_eq_gcd_ab f' g_E,
    --     simp only [polynomial.eval_add,polynomial.eval_sub,polynomial.eval_mul,polynomial.eval_C,polynomial.eval_X,polynomial.eval_comp,polynomial.eval_map,←polynomial.aeval_def,minimal_polynomial.aeval,add_sub_cancel,zero_mul,add_zero],
    -- end,
    -- have h_splits : polynomial.splits ι_FFγ h :=
    --     polynomial.splits_of_splits_of_dvd (algebra_map E E') (polynomial.map_ne_zero (minimal_polynomial.ne_zero hβ)) (polynomial.splitting_field.splits g_E) (euclidean_domain.gcd_dvd_right f' g_E),
    -- have h_roots : ∀ x : roots h E', ↑x = algebra_map E E' β :=
    -- begin
    --     intro x,
